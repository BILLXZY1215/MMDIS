//input:1000個測資檔，每個測資有30個點（行）
//output:1000行，每行代表30個點裡面有幾個點加入set中
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#define NODE  30
...
int main()
{
    ...
    double openNodesCount = 0;
    double averageCount = 0;
    //char filename2[1000]={0}; 
    FILE *tinput,*fout, *averageCountFile;
    //FILE *tinput,*rinput,*fout;
    int rinput = 1; // k = 1
    srand((unsigned)time(NULL));
    fout=fopen("MMDIS_calculate.txt","w"); // output file   
    averageCountFile = fopen("calculate/MMDIS_averageCountER1.txt","a"); // output file  
    //fout=fopen("\\Users\\shih\\eclipse-workspace\\MMDS_code\\minimal multi-domination(k不同)\\500_WS\\activity_num100(P=0,mk=2).txt","w");     
    /*
    char topologyPath = "";
    char types[4] = {"BA/", "ER/", "UDG/", "WS/"};
    char N[2] = {"30/", "50/"};
    char parameter[5] = {"1/", "2/", "3/", "4/", "5/"};
    */
    for (value = 1; value <= file_num; value++)
    {
        //topologyPath = "topology/" + types[0] + N[0] + parameter[1] + value;
        printf("graph%d開始run\n",value);                          
//        sprintf(filename1,"topology/BA/30/1/%d.txt",value);    // 13.77 讀入topology  
//        sprintf(filename1,"topology/BA/30/2/%d.txt",value);    // 9.81 讀入topology  
//        sprintf(filename1,"topology/BA/30/3/%d.txt",value);    // 8.04 讀入topology  
//        sprintf(filename1,"topology/BA/30/4/%d.txt",value);    // 6.85 讀入topology  
//        sprintf(filename1,"topology/BA/30/5/%d.txt",value);    // 5.94 讀入topology  沒有924.txt，925代替

        sprintf(filename1,"topology/ER/30/1/%d.txt",value);    // 11.29 讀入topology  
//        sprintf(filename1,"topology/ER/30/2/%d.txt",value);    // 7.87 讀入topology  
//        sprintf(filename1,"topology/ER/30/3/%d.txt",value);    // 5.84 讀入topology  
//        sprintf(filename1,"topology/ER/30/4/%d.txt",value);    // 4.68 讀入topology  
//        sprintf(filename1,"topology/ER/30/5/%d.txt",value);    // 3.83 讀入topology  

//        sprintf(filename1,"topology/UDG/50/%d.txt",value);    //  讀入topology  

//        sprintf(filename1,"topology/WS/30/1/%d.txt",value);    // 8.13 讀入topology  
//        sprintf(filename1,"topology/WS/30/2/%d.txt",value);    // 8.35 讀入topology  
//        sprintf(filename1,"topology/WS/30/3/%d.txt",value);    // 8.92 讀入topology  
//        sprintf(filename1,"topology/WS/30/4/%d.txt",value);    // 9.22 讀入topology  
//        sprintf(filename1,"topology/WS/30/5/%d.txt",value);    // 9.52 讀入topology 
...
//printf("gameover\n");  
    ...
    {
        openNodesCount += open_num[i];
        fprintf(fout,"%d\n",open_num[i]);
    }  
    averageCount = openNodesCount / file_num;
    fprintf(averageCountFile,"%f\n",averageCount);
    printf("nodes count: %f\n",averageCount); // 算平均 open node number 
    printf("%d\n",index);   
    fclose(averageCountFile);  
    fclose(fout); 
